...
async function initAwarenessSwiper() {
    try {
        ...
        const locale = pathName.includes('/en/') ? 'en' : pathName.includes('/al/') ? 'al' : 'mk';
                    ...
                    <div class="aw-card">
                        <div class="flex flex-col aw-card-body bg-white/60 rounded-xl p-[3%] xs:p-[4%] 2xl:p-[2%] gap-[2vh]">
                            <div class="flex items-center gap-[1vh] 2xl:gap-[2vh]">
                                <img src="${expert.image}" class="h-[6vh] w-[6vh] 2xl:h-[10vh] 2xl:w-[10vh] object-contain object-center rounded-full">
                                <div class="flex flex-col text-black/85 font-bold text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl 2xl:text-3xl 3xl:text-4xl">
                                    <div class="flex gap-1"><p>${expert.name}</p><p>${expert.surname}</p></div>
                                    <div class="flex text-nowrap gap-1 font-bold"><p>${title}</p><p class="hidden xs:flex">${connectorText}</p><p class="hidden xs:flex">${organization}</p></div>
                                </div>
                            </div>
                            ${quotes.map(q => `<p class="text-sm md:text-lg xl:text-xl 2xl:text-2xl 3xl:text-3xl text-black/75">${q}</p>`).join('')}
                            <p class="text-black/50 text-xs sm:text-sm 2xl:text-base 3xl:text-lg pt-[3vh]">${attributionText} <a target="_blank" class="text-black/75 hover:text-black transition-all duration-300" href="${expert.source_link}">${expert.source_publication}</a></p>
                        </div>
                    </div>
                `;
            }
